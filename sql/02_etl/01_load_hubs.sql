...
BEGIN
    ...
    SELECT
        MD5(UPPER(TRIM(CAST(customer_id AS TEXT)))),
    ...
    WHERE NOT EXISTS (
        ...
        WHERE h.customer_bk = CAST(src.customers.customer_id AS TEXT)
    );
    ...
    SELECT
        MD5(UPPER(TRIM(product_code))),
        product_code,
        created_at,
        'src.products'
    FROM src.products
    WHERE NOT EXISTS (
        SELECT 1 FROM dv.hub_product h
        WHERE h.product_bk = src.products.product_code
    );
    GET DIAGNOSTICS v_rows = ROW_COUNT;
    ...
    SELECT
        MD5(UPPER(TRIM(CAST(order_id AS TEXT)))),
        CAST(order_id AS TEXT),
        created_at,
        'src.orders'
    FROM src.orders
    WHERE NOT EXISTS (
        SELECT 1 FROM dv.hub_order h
        WHERE h.order_bk = CAST(src.orders.order_id AS TEXT)
    );
    GET DIAGNOSTICS v_rows = ROW_COUNT;
